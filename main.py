...
tk.Button(root, text="Run Grain Counting", command=grainCounting.run_grain_counting).grid(row=9, column=0, pady=10)

# Add a button to reset the parameters to their default values
tk.Button(root, text="Reset Values", command=grainCounting.reset_values).grid(row=9, column=2, pady=10)

# Add a button to select a file
tk.Button(root, text="Select File", command=grainCounting.select_file).grid(row=9, column=1, pady=10, padx=10)
