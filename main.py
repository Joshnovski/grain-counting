...
tk.Label(root, text="Smaller Grain Area Min:").grid(row=4, column=0, sticky='E')
tk.Entry(root, textvariable=config.smaller_grain_area_min).grid(row=4, column=2)

tk.Label(root, text="Smaller Grain Area Max:").grid(row=5, column=0, sticky='E')
tk.Entry(root, textvariable=config.smaller_grain_area_max).grid(row=5, column=2)

tk.Label(root, text="Larger Grain Area Min:").grid(row=6, column=0, sticky='E')
tk.Entry(root, textvariable=config.larger_grain_area_min).grid(row=6, column=2)

tk.Label(root, text="Larger Grain Area Max:").grid(row=7, column=0, sticky='E')
tk.Entry(root, textvariable=config.larger_grain_area_max).grid(row=7, column=2)
...
tk.Button(root, text="Run Grain Counting", command=grainCounting.run_grain_counting).grid(row=8, column=0, pady=10)

# Add a button to reset the parameters to their default values
tk.Button(root, text="Reset Values", command=grainCounting.reset_values).grid(row=8, column=2, pady=10)

# Add a button to select a file
tk.Button(root, text="Select File", command=grainCounting.select_file).grid(row=8, column=1, pady=10, padx=10)
